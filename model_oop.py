...
class RainModel(ABC):
    ...
    @abstractmethod
    def __prediction(self, input_data):
        pass
    ...
    def __CSI_m(self, y_true, y_pred):
    ...
    def __CSI_custom(self, y_true, y_pred):
        score = tf.py_function(
            func=self.__CSI_m, inp=[y_true, y_pred], Tout=tf.float32, name="CSI_custom"
        )
        return score

    def __mae_custom(self, y_true, y_pred):
        y_pred = tf.convert_to_tensor(y_pred)
        y_true = tf.convert_to_tensor(y_true)
        y_true = self.__rain(y_true)
        y_pred = self.__rain(y_pred)
    ...
    def __data_preprocessing(self, X):
        # 0. Right shape for batch
        X = X[np.newaxis, ::, ::, ::]
        return X

    def __data_postprocessing(self, nwcst):
    ...
    def __save_as_ascii_grid(self, data, output_file):
        """
...
class RainVer1Model(RainModel):
    ...
    def save_prediction_files(self, input_data: np.ndarray, output_folder_path: str):
        ...
        for j in range(10, 190, 10):
            ...
            self.__save_as_ascii_grid(qpf_np, output_file_path)

    def __load_model(self, path: str):
        model = tf.keras.models.load_model(
            path,
            custom_objects={
                "CSI_custom": self.__CSI_custom,
                "mae_custom": self.__mae_custom,
            },
        )
        return model

    def __prediction(self, input_data: np.ndarray):
        processed_data = self.__data_preprocessing(input_data)
...
class RainVer2Model(RainModel):
    ...
    def save_prediction_files(self, input_data: np.ndarray, output_folder_path: str):
        ...
        for j in range(10, 190, 10):
            ...
            k = int(j / 10 - 1)
            self.__save_as_ascii_grid(prediction[k], output_file_path)
    ...
    def __zip_model(self, path: str):
        ...
        for i in range(10, 190, 10):
            pre_model = f"{path}/model-best_fcst_{j}min.tflite"
    ...
    def __prediction(self, input_data: np.ndarray):
        ...
        for model in self.model:
            ...
            model.invoke()
            nwcst.append(model.get_tensor(output_details[0]["index"]))
        return nwcst
